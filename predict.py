...
def saveFddbData(path, file_name):
    ...
    for item in file_list:
        ...
        for i, (box) in enumerate(boxes):
            ...
            y1 = box[1] * h
            y2 = box[3] * h
            f_write.write(str(x1) + '\t' + str(y1) + '\t' + str(x2 - x1) + '\t' + str(y2 - y1) + '\t' + str(
                probs[i]) + '\t' + '1\n')
    f_write.close()
...
if __name__ == '__main__':
    ...
    path = os.path.join(BASEPATH + "/datasets/video/source_1280x720_1.flv")
    testVideo(path)

    # given image path, predict and show
    # root_path = os.path.join(BASEPATH + "/datasets/widerFace/WIDER_train/images/0--Parade/")
    # picture = '0_Parade_marchingband_1_495.jpg'
    # testIm(root_path + picture)

    # given image path, predict and show
    # fddb_path = os.path.join(BASEPATH + "/datasets/fddb/originalPics/2002/07/19/big/")
    # picture = 'img_463.jpg'
    # im = testIm(fddb_path + picture)
    # cv2.imwrite('picture/img_detected', im)

    # given image file list, predict and show
    # path = os.path.join(BASEPATH + "/datasets/fddb/originalPics/")
    # file_name = os.path.join(BASEPATH + "/datasets/fddb/FDDB-folds/FDDB-fold-01.txt")
